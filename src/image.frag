...
vec2 map(vec3 p) {
    return opU(vec2(sdPlane(p), 1.0), menger(p - vec3(0, 1, 0)));
}
...
mat4 setCamera(vec3 origin, vec3 target, float rotation) {
    vec3 cw = normalize(target - origin);
    vec3 cp = vec3(sin(rotation), cos(rotation), 0.0);
    vec3 cu = normalize(cross(cw, cp));
    vec3 cv = normalize(cross(cu, cw));
    return mat4(vec4(cu, 0), vec4(cv, 0), vec4(cw, 0), vec4(0, 0, 0, 1));
}
